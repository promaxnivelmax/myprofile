    ...
    return (
      <div className="max-w-md mx-auto my-20 p-10 bg-white dark:bg-zinc-900 rounded-[2.5rem] shadow-2xl animate-fade-in-up border border-zinc-100 dark:border-zinc-800">
        <div className="text-center mb-10">
          <div className="w-20 h-20 bg-[#D4AF37] text-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl gold-glow">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <input 
            ...
            className="w-full p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 dark:bg-zinc-950 outline-none text-center font-black tracking-widest text-[#D4AF37]"
            placeholder="••••••••"
          />
          <button type="submit" className="w-full bg-black dark:bg-white text-white dark:text-black font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-[10px] hover:opacity-90 transition-all">
            Desbloquear Panel
          </button>
        </form>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 animate-fade-in-up">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <h1 className="text-4xl font-black uppercase tracking-tighter">Panel de <span style={{ color: '#D4AF37' }}>Gestión</span></h1>
          ...
          {(['profile', 'experience', 'education', 'skills', 'projects', 'supports'] as const).map(tab => (
            <button
              ...
              className={`w-full text-left px-6 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${
                activeTab === tab ? 'bg-black text-white dark:bg-white dark:text-black shadow-lg scale-105' : 'text-zinc-400 hover:text-black dark:hover:text-white'
              }`}
            >
          ...
          {activeTab === 'experience' && (
            <div className="space-y-10">
              <div className="flex justify-between items-center border-b pb-4 border-zinc-100 dark:border-zinc-800">
                ...
                <button onClick={() => addItem('experience')} className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-xl text-xs font-black uppercase">+ Agregar</button>
              </div>
          ...
          {activeTab === 'education' && (
            <div className="space-y-10">
              <div className="flex justify-between items-center border-b pb-4 border-zinc-100 dark:border-zinc-800">
                ...
                <button onClick={() => addItem('education')} className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-xl text-xs font-black uppercase">+ Agregar</button>
              </div>
          ...
          {activeTab === 'skills' && (
            <div className="space-y-10">
              <div className="flex justify-between items-center border-b pb-4 border-zinc-100 dark:border-zinc-800">
                ...
                <button onClick={() => addItem('skills')} className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-xl text-xs font-black uppercase">+ Agregar</button>
              </div>
                ...
                {formData.skills.map(skill => (
                  <div key={skill.id} className="p-6 bg-zinc-50 dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="flex items-center gap-4">
                      <input type="range" min="0" max="100" value={skill.level} onChange={(e) => updateItem('skills', skill.id, 'level', parseInt(e.target.value))} className="flex-grow accent-[#D4AF37]" />
          ...
          {activeTab === 'projects' && (
            <div className="space-y-10">
              <div className="flex justify-between items-center border-b pb-4 border-zinc-100 dark:border-zinc-800">
                ...
                <button onClick={() => addItem('projects')} className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-xl text-xs font-black uppercase">+ Agregar</button>
              </div>
          ...
          {activeTab === 'supports' && (
            <div className="space-y-10">
              <div className="flex justify-between items-center border-b pb-4 border-zinc-100 dark:border-zinc-800">
                ...
                <button onClick={() => addItem('supports')} className="p-3 bg-black dark:bg-white text-white dark:text-black rounded-xl text-xs font-black uppercase">+ Agregar</button>
              </div>
                ...
                <div key={sup.id} className="p-8 bg-zinc-50 dark:bg-zinc-950 rounded-[2rem] border border-zinc-100 dark:border-zinc-800 space-y-6">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select className="w-full p-3 border rounded-xl dark:bg-zinc-900 dark:border-zinc-800" value={sup.category} onChange={(e) => updateItem('supports', sup.id, 'category', e.target.value)}>
