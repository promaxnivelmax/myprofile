    ...
    <div className="max-w-7xl mx-auto px-4 py-16 animate-fade-in-up">
      <div className="text-center mb-20">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto font-bold text-lg">
          Soluciones inteligentes diseñadas para simplificar trámites y potenciar la eficiencia administrativa.
        </p>
      </div>
          ...
          <div key={project.id} className="bg-white dark:bg-zinc-900 rounded-[3rem] shadow-2xl hover:shadow-accent/20 transition-all duration-700 border border-zinc-100 dark:border-zinc-800 overflow-hidden flex flex-col group hover:-translate-y-4">
            ...
            <div className="p-12 flex-grow flex flex-col">
              <div className="flex justify-between items-start mb-8">
                <span className="px-5 py-2 bg-zinc-100 dark:bg-zinc-800 rounded-full text-[10px] font-black uppercase tracking-widest text-zinc-500">
                  {project.category}
                </span>
                <i className="fas fa-magic text-3xl opacity-20 group-hover:opacity-100 transition-all group-hover:scale-125 text-accent"></i>
              </div>
              <h3 className="text-3xl font-black mb-6 leading-tight uppercase tracking-tight group-hover:text-accent transition-colors">{project.title}</h3>
              ...
              <div className="mt-auto">
                {project.link && (
                  <a 
                    href={project.link} 
                    target="_blank" 
                    className="flex items-center justify-center gap-4 w-full py-5 px-8 rounded-3xl bg-zinc-900 dark:bg-white text-white dark:text-black font-black text-xs uppercase tracking-widest hover:bg-accent dark:hover:bg-accent transition-all active:scale-95 shadow-xl"
                  >
      ...
      <div className="mt-24 p-16 bg-white dark:bg-zinc-900 rounded-[3.5rem] shadow-2xl border-4 border-accent text-center gold-glow relative overflow-hidden">
         <div className="absolute -bottom-10 -right-10 opacity-5">
            <i className="fas fa-handshake text-[15rem]"></i>
         </div>
         <h4 className="text-3xl font-black mb-4 uppercase tracking-tighter">¿Buscas una solución a tu medida?</h4>
         <p className="text-zinc-500 dark:text-zinc-400 mb-10 text-lg font-medium max-w-xl mx-auto">Desarrollemos juntos la herramienta tecnológica que tu oficina o negocio necesita.</p>
         <a 
         ...
         >
            <i className="fab fa-whatsapp text-2xl"></i> HABLAR POR WHATSAPP
         </a>
      </div>
    </div>
  );
